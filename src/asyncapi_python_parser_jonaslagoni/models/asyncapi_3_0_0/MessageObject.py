...
class MessageObject(BaseModel): 
  ...
  bindings: Optional[Union[Reference.Reference, MessageBindingsObject.MessageBindingsObject]] = Field(default=None)
  traits: Optional[List[Reference.Reference | MessageTrait.MessageTrait | List[Reference.Reference | MessageTrait.MessageTrait | dict[str, Any] | Any]]] = Field(description='''A list of traits to apply to the message object. Traits MUST be merged using traits merge mechanism. The resulting object MUST be a valid Message Object.''', default=None)
  ...
  def unwrap_extensions(cls, data):
    ...
    known_object_properties = ['content_type', 'headers', 'payload', 'correlation_id', 'tags', 'summary', 'name', 'title', 'description', 'external_docs', 'deprecated', 'examples', 'bindings', 'traits', 'extensions']
    unknown_object_properties = [element for element in json_properties if element not in known_object_properties]
    # Ignore attempts that validate regular models, only when unknown input is used we add unwrap extensions
    if len(unknown_object_properties) == 0: 
      return data
  
    known_json_properties = ['contentType', 'headers', 'payload', 'correlationId', 'tags', 'summary', 'name', 'title', 'description', 'externalDocs', 'deprecated', 'examples', 'bindings', 'traits', 'extensions']
    extensions = {}
