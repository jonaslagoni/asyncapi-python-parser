...
class Oauth2Flows(BaseModel): 
  ...
  scopes: Optional[List[str]] = Field(description='''List of the needed scope names.''', default=None)
  ...
  def unwrap_extensions(cls, data):
    ...
    known_object_properties = ['type', 'description', 'flows', 'scopes', 'extensions']
    unknown_object_properties = [element for element in json_properties if element not in known_object_properties]
    # Ignore attempts that validate regular models, only when unknown input is used we add unwrap extensions
    if len(unknown_object_properties) == 0: 
      return data
  
    known_json_properties = ['type', 'description', 'flows', 'scopes', 'extensions']
    extensions = {}
