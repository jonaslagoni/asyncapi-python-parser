...
class Message(BaseModel): 
  ...
  def unwrap_extensions(cls, data):
    ...
    known_object_properties = ['schema_format', 'content_type', 'headers', 'payload', 'correlation_id', 'tags', 'summary', 'name', 'title', 'description', 'external_docs', 'deprecated', 'examples', 'protocol_info', 'traits', 'extensions']
    unknown_object_properties = [element for element in json_properties if element not in known_object_properties]
    # Ignore attempts that validate regular models, only when unknown input is used we add unwrap extensions
    if len(unknown_object_properties) == 0: 
      return data
  
    known_json_properties = ['schemaFormat', 'contentType', 'headers', 'payload', 'correlationId', 'tags', 'summary', 'name', 'title', 'description', 'externalDocs', 'deprecated', 'examples', 'protocolInfo', 'traits', 'extensions']
    extensions = {}
