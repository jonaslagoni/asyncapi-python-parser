...
class Components(BaseModel): 
  ...
  def unwrap_extensions(cls, data):
    ...
    known_object_properties = ['schemas', 'servers', 'channels', 'messages', 'security_schemes', 'parameters', 'correlation_ids', 'operation_traits', 'message_traits', 'server_bindings', 'channel_bindings', 'operation_bindings', 'message_bindings', 'extensions']
    unknown_object_properties = [element for element in json_properties if element not in known_object_properties]
    # Ignore attempts that validate regular models, only when unknown input is used we add unwrap extensions
    if len(unknown_object_properties) == 0: 
      return data
  
    known_json_properties = ['schemas', 'servers', 'channels', 'messages', 'securitySchemes', 'parameters', 'correlationIds', 'operationTraits', 'messageTraits', 'serverBindings', 'channelBindings', 'operationBindings', 'messageBindings', 'extensions']
    extensions = {}
