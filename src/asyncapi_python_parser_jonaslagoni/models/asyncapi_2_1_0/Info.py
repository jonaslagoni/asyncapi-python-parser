...
class Info(BaseModel): 
  ...
  contact: Optional[Contact.Contact] = Field(description='''Contact information for the owners of the API.''', default=None)
  ...
  def unwrap_extensions(cls, data):
    ...
    known_object_properties = ['title', 'version', 'description', 'terms_of_service', 'contact', 'license', 'extensions']
    unknown_object_properties = [element for element in json_properties if element not in known_object_properties]
    # Ignore attempts that validate regular models, only when unknown input is used we add unwrap extensions
    if len(unknown_object_properties) == 0: 
      return data
  
    known_json_properties = ['title', 'version', 'description', 'termsOfService', 'contact', 'license', 'extensions']
    extensions = {}
